...
def execute_txt2img(
    ...
    output = (
        "/tmp/"
        + shuffle_string(
            str(uuid.uuid4()).split("-")[-1] + str(time.time()).replace(".", "")
        )
        + ".png"
    )
    ...
    cmd = [
        ...
        str(output),
        "--prompt",
        str(diffuser_model.prompt),
        "--negative-prompt",
        str(diffuser_model.negative_prompt),
        "--cfg-scale",
        str(diffuser_model.cfg_scale),
        ...
        "--steps",
        str(diffuser_model.sample_steps),
        # "--rng", str(diffuser_model.rng_type),
        "--seed",
        str(diffuser_model.seed),
        # "--schedule", str(diffuser_model.schedule),
        "--upscale-repeats",
        str(diffuser_model.upscale_repeats),
    ]
    ...
    print(cmd)
    try:
        return execute_and_return_image(
            cmd, output, diffuser_model.use_xbrz, diffuser_model.xbrz_scale
        )
...
async def upload_file_to_s3(file: UploadFile = File(...)):
    ...
    s3 = boto3.client("s3")
    unique_filename = shuffle_string(
        str(uuid.uuid4()).replace("-", "") + str(time.time()).replace(".", "")
    )
